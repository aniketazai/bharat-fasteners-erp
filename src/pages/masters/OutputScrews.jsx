...
export default function OutputScrews() {
  ...
  const [form, setForm]             = useState({ screw_name: '' })
  ...
  function nextCode() {
    const max = records.reduce((m, r) => {
      const n = parseInt(r.screw_code?.replace(/\D/g, '') || '0')
      return n > m ? n : m
    }, 0)
  ...
  async function handleAdd(e) {
    ...
    setSaving(false)
    if (error) { setFormErrors({ screw_name: error.message }); return }
    setForm({ screw_name: '' })
    setShowForm(false)
    load()
  }
  ...
  function startEdit(row) {
    setEditId(row.id)
    setEditData({ screw_name: row.screw_name })
    setEditErrors({})
  }
  ...
  return (
    <div className="main page-enter">
      ...
      <div className="stats" style={{ gridTemplateColumns: 'repeat(3,1fr)', maxWidth: 480 }}>
        <div className="stat"><div className="stat-n">{records.length}</div><div className="stat-l">Total Screws</div></div>
        <div className="stat" style={{ borderLeftColor: 'var(--green)' }}>
          <div className="stat-n" style={{ color: 'var(--green)' }}>{active}</div><div className="stat-l">Active</div>
        </div>
        <div className="stat" style={{ borderLeftColor: 'var(--dim)' }}>
          <div className="stat-n" style={{ color: 'var(--muted)' }}>{inactive}</div><div className="stat-l">Inactive</div>
        </div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, gap: 10 }}>
        <input
          style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 5, padding: '7px 12px', fontSize: 13, color: 'var(--text)', outline: 'none', width: 220 }}
          placeholder="Search screw name…"
          value={search}
          onChange={e => setSearch(e.target.value)}
        />
        <button className="btn-add" onClick={() => { setShowForm(v => !v); setFormErrors({}); setForm({ screw_name: '' }) }}>
        ...
        <div className="form-card">
          ...
          <form onSubmit={handleAdd}>
            ...
            <div className="form-actions">
              <button className="btn-add" type="submit" disabled={saving}>{saving ? 'SAVING…' : 'SAVE SCREW'}</button>
              <button className="btn-clear" type="button" onClick={() => { setShowForm(false); setForm({ screw_name: '' }); setFormErrors({}) }}>CANCEL</button>
            </div>
          </form>
        </div>
      )}
        ...
        <table>
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={4} className="empty">{search ? 'No screws match your search.' : 'No screws found.'}</td></tr>
            )}
            {filtered.map((row, i) => (
              <tr key={row.id}>
